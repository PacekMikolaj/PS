...
class USOS:
    ...
    def __init__(self):
        self.grades = []
        self.students = dict[Student, list[Grade]]

    def add(self, student_name, subject_name, mark):
        student = None
        for index in self.students:
            if index == student_name:
                student = index
                break
        if student == None:
        # if len(self.students) <= student_index:
            ...
            self.students[student] =  list[Grade]
        ...
        if subject == None:
            print('Brak oceny w bazie!')
            return
        grade = Grade(subject,mark)
